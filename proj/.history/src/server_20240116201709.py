...
def send_or_update_entity(entity):
    ...
    entity_attributes = entity['attributes']  # Ensure correct assignment of attributes

    headers = {"Accept": "application/ld+json"}
    get_response = requests.get(f"{ORION_URL}/entities/{entity_id}", headers=headers)
    
    try:
        if get_response.status_code == 200:
            # The entity already exists, so we update it
            update_headers = {"Content-Type": "application/ld+json"}
            update_response = requests.patch(f"{ORION_URL}/entities/{entity_id}/attrs",
                                             json=entity_attributes, headers=update_headers)
            ...
            create_response = requests.post(f"{ORION_URL}/entities",
                                            json={
                                                "id": entity_id,
                                                "type": entity_type,
                                                "attributes": entity_attributes
            ...
            if create_response.status_code not in [201, 204]:
                print(f"Error creating entity {entity_id}: Status Code {create_response.status_code}")
                print(f"Response: {create_response.text}")
            else:
                print(f"Successfully created entity {entity_id}")
        else:
            print(f"Error checking existence of entity {entity_id}: {get_response.status_code}")
        ...
        if 'update_response' in locals() and update_response.content:
            print(f"Update response content: {update_response.content}")
        if 'create_response' in locals() and create_response.content:
