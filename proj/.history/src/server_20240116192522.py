...
def send_or_update_entity(entity):
    entity_id = entity['id']
    # For GET requests, we only need the Accept header
    get_headers = {"Accept": "application/json"}
    get_response = requests.get(f"{ORION_URL}/{entity_id}", headers=get_headers)
    
    if get_response.status_code == 200:
        # The entity already exists, so we update it
        update_headers = {"Content-Type": "application/json"}
        # Prepare the payload without the id and type for the PATCH request
        entity_attributes = entity['attributes']
        update_response = requests.patch(f"{ORION_URL}/entities/{entity_id}/attrs", json=entity_attributes, headers=update_headers)
        ...
        # Include the id and type for the POST request
        create_response = requests.post(f"{ORION_URL}/entities", json={
            "id": entity_id,
            "type": entity['type'],
            **entity_attributes
